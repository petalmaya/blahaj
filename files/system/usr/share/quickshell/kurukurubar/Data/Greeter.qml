...
Singleton {
  ...
  Process {
    ...
    command: ["bash", "-c", "bash " + Dat.Paths.urlToPath(Qt.resolvedUrl("../scripts/session.sh")) + " /usr/share/wayland-sessions 2>/dev/null; bash " + Dat.Paths.urlToPath(Qt.resolvedUrl("../scripts/session.sh")) + " /usr/share/xsessions 2>/dev/null"]
    running: true
